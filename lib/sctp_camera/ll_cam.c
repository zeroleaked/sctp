...
bool ll_cam_dma_sizes(cam_obj_t *cam){
    ...
    size_t node_max = LCD_CAM_DMA_NODE_BUFFER_MAX_SIZE / cam->dma_bytes_per_item; // 4092
    size_t line_width = cam->width * cam->in_bytes_per_pixel; // 2048
    size_t node_size = node_max; // 4092
    ...
    if(line_width >= node_max){
        ESP_LOGI(TAG, "One or more nodes will be requied for one line");
        // One or more nodes will be requied for one line
        for(size_t i = node_max; i > 0; i=i-1){
            if ((line_width % i) == 0) {
                node_size = i;
                ESP_LOGD(TAG, "line_width=%d, node_size=%d", line_width, node_size);
        ...
        ESP_LOGI(TAG, "One or more lines can fit into one node");
        // One or more lines can fit into one node
        for(size_t i = node_max; i > 0; i=i-1){
            if ((i % line_width) == 0) {
                node_size = i; // 2048
                lines_per_node = node_size / line_width; // 1
...
esp_err_t ll_cam_set_pin(cam_obj_t *cam, const camera_config_t *config)
{
    ...
    //     config->pin_d5, config->pin_d6, config->pin_d7, config->pin_d8, config->pin_d9
    // };
    int data_pins[10] = {
        config->pin_d9, config->pin_d8, config->pin_d7, config->pin_d6, config->pin_d5,
        config->pin_d4, config->pin_d3, config->pin_d2, config->pin_d1, config->pin_d0
    };
    for (int i = 0; i < 16; i++) {
        if (i < 6) {
            gpio_matrix_in(0x3C, CAM_DATA_IN0_IDX + i, false);
        } else {
            PIN_FUNC_SELECT(GPIO_PIN_MUX_REG[data_pins[i-6]], PIN_FUNC_GPIO);
            gpio_set_direction(data_pins[i-6], GPIO_MODE_INPUT);
            gpio_set_pull_mode(data_pins[i-6], GPIO_FLOATING);
            gpio_matrix_in(data_pins[i-6], CAM_DATA_IN0_IDX + i, false);
        }
    }
...
static esp_err_t ll_cam_dma_init(cam_obj_t *cam)
{
    for (int x = (SOC_GDMA_PAIRS_PER_GROUP - 1); x >= 0; x--) {
        ...
        if (x == 0) {
            // cam_deinit();
            ESP_LOGE(TAG, "No available GDMA channel");
			return ESP_FAIL;
        }
    }

    // enable DMA clock and reset dma
    ...
    // reset RX FSM and RX FIFO pointer
...
esp_err_t ll_cam_config(cam_obj_t *cam, const camera_config_t *config)
{
    ...
    LCD_CAM.cam_ctrl.cam_clkm_div_num = 160000000 / config->xclk_freq_hz / 2;
    LCD_CAM.cam_ctrl.cam_clk_sel = 3;//Select Camera module source clock. 0: no clock. 1: XTAL_CLK. 2: PLL_D2_CLK. 3: PLL_F160M_CLK.

    LCD_CAM.cam_ctrl.cam_stop_en = 0; 
    // LCD_CAM.cam_ctrl.cam_vsync_filter_thres = 2; // Filter by LCD_CAM clock
    ...
    LCD_CAM.cam_ctrl.cam_bit_order = 1;
    LCD_CAM.cam_ctrl.cam_line_int_en = 0;
    LCD_CAM.cam_ctrl.cam_vs_eof_en = 0; //1: CAM_VSYNC to generate in_suc_eof. 0: in_suc_eof is controlled by cam_rec_data_bytelen

    LCD_CAM.cam_ctrl1.val = 0;
    LCD_CAM.cam_ctrl1.cam_rec_data_bytelen = LCD_CAM_DMA_NODE_BUFFER_MAX_SIZE - 1; // Set later in ll_cam_start
